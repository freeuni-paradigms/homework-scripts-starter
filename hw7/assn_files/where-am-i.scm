...
(define (k-subsets set k)
  (cond 
    ((eq? (length set) k) (list set))
    ((zero? k) '(()))
    ((or (negative? k) (> k (length set))) '())
    (else 
      (let 
        (
          (k-subsets-of-rest (k-subsets (cdr set) k))
          (k-1-subsets-of-rest (k-subsets (cdr set) (- k 1)))
        )
        (append 
          (map (lambda (subset) (cons (car set) subset)) k-1-subsets-of-rest)
          k-subsets-of-rest
        )
      )
    )
...
(define (partition pivot ls fn)
  (if (null? ls) '(() ())
      (let ((split-of-rest (partition pivot (cdr ls) fn)))
	(if (< (fn (car ls)) (fn pivot))
	    (list (cons (car ls) (car split-of-rest)) (cadr split-of-rest))
	    (list (car split-of-rest) (cons (car ls) (car (cdr split-of-rest))))))))
...
(define (distance-product point points)
  (apply * 1 
    (map
      (lambda (other-point) (dist point other-point))
      (remove point points)
    )
  )
)
...
(define (clumped-points points)
  (prefix-of-list 
    (map
      (lambda (rated-point) (cadr rated-point)) 
...
(define (average-point points)
  (let
    (
      (average-x
        (/ 
          (apply + (map (lambda (point) (car point)) points)) 
          (length points)
        )
      )
      (average-y
        (/ 
          (apply + (map (lambda (point) (cadr point)) points)) 
          (length points)
        )
      )
    )
    (list
      (distance-product (list average-x average-y) points)
      (list average-x average-y)
    )
  )
)
